...
WORKDIR /src
COPY ["dotnet-lambda-container-sample.csproj", "dotnet-lambda-container-sample/"]
RUN dotnet restore "dotnet-lambda-container-sample/dotnet-lambda-container-sample.csproj"

WORKDIR "/src/dotnet-lambda-container-sample"
COPY . .
RUN dotnet build "dotnet-lambda-container-sample.csproj" --configuration Release --output /app/build
...
Execute unit tests
```
cd "dotnet-lambda-container-sample/test/dotnet-lambda-container-sample.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "dotnet-lambda-container-sample/src/dotnet-lambda-container-sample"
